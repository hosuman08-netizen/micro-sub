...
(function(){
  ...
  root.innerHTML='<div class="card">현재 티어: <b style="color:var(--gold)">'+cur+'</b> · 가상 · 정진 · Elite 창 '+eliteLeft()+'</div>'+tiers.map(function(t){
    ...
    root.querySelectorAll('button[data-t]').forEach(function(b){b.onclick=function(){cur=b.dataset.t;localStorage.setItem('msc_tier',cur); try{var th=JSON.parse(localStorage.getItem('msc_hist')||'[]');th.unshift({t:cur,ts:Date.now()});localStorage.setItem('msc_hist',JSON.stringify(th.slice(0,20)));}catch(e){} render();try{legionTrack('activate',{tier:cur})}catch(e){};};});
